...
function Obj({ auth, item, options = {}, value, onChange, changed }) {
    ...
    useEffect(() => {
        if (item && item.relation && item.relation.reference && item.relation.reference.url) {
            ...
            }, options.detail || 'model', item.filter || '', item.relation.reference.object);
        }
    }, []);
    ...
    const virtualizedItem = (item, idx) => {
        if (item && !item.view || (item && item.view && item.view.unvisible === false)) {
            return (
                ...
                >
                    <Field className={classes.Field} auth={auth} form={form} meta={meta} item={item} options={options} onChange={onFieldChange} changed={changed}></Field>
                </Form.Item>
            );
        }
    };
